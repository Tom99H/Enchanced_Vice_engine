...
int ParseFen(char *fen, S_BOARD *pos) {
    ...
    // Add occupied bitboards initialization
    pos->occupied[WHITE] = 0ULL;
    pos->occupied[BLACK] = 0ULL;
    pos->occupied[BOTH] = 0ULL;
    for (int sq64 = 0; sq64 < 64; ++sq64) {
        int sq120 = SQ120(sq64);
        int piece = pos->pieces[sq120];
        if (piece != EMPTY) {
            int col = PieceCol[piece];
            pos->occupied[col] |= (1ULL << sq64);
            pos->occupied[BOTH] |= (1ULL << sq64);
        }
    }
